-- Seed script to create schema and load 100k synthetic rows for local testing
-- Run with: psql -U <user> -d <database> -f backend/db/seed-100k.sql
...
CREATE TABLE IF NOT EXISTS password_reset_tokens (
    ...
    CONSTRAINT fk_password_reset_tokens_user FOREIGN KEY (user_account_id) REFERENCES user_accounts (id) ON DELETE CASCADE
);
...
-- Clean existing data so the script can be rerun safely
TRUNCATE TABLE messages, reviews, trades, item_wishlist, item_images, items,
    refresh_tokens, password_reset_tokens, user_accounts, profiles, categories,
...
-- Generate an extra 100k synthetic records
DO $$
DECLARE
    i INTEGER;
    v_profile_id UUID;
    ...
    category_ids UUID[] := ARRAY[
        '3b28c8ee-7724-4eab-955e-a68b7ec1d356',
        '1a2d0e53-dc40-431d-aa4a-c90098405a4c',
        '8a3dc0fb-ac14-4c24-9f7e-1b18eb89c75e',
        '92312e90-ce77-49d2-b3cd-685b4e27983a',
        '8167d8f5-d7ef-4078-83f7-61a83a2564ae',
        '7a94b32c-e8f8-477c-b367-189872234263',
        '52543f69-94f1-44f9-ae91-9e0d8c83dc7e',
        'bab5a72d-ac73-4f6d-940a-44a84188db67'
    ];
    hash_password TEXT := '$2b$12$aBTtH4Q5cHn7pLnachcReemx5StqD.L.6haQnbxAy48DCo7yITLoe';
...
BEGIN
    FOR i IN 1..100000 LOOP
        ...
        v_category := category_ids[(i % array_length(category_ids, 1)) + 1];
        created_time := now() - make_interval(hours => (i % 240));

        INSERT INTO profiles (id, display_name, bio, avatar_url, location, phone, rating, total_trades, created_at, updated_at)
        VALUES (
            v_profile_id,
            format('Usuario Demo %s', i),
            format('Perfil de prueba número %s para pruebas de carga.', i),
            'https://images.unsplash.com/photo-1524504388940-b1c1722653e1',
            format('Ciudad %s', i % 200),
            format('+1-555-000-%s', lpad(i::text, 5, '0')),
        ...
        INSERT INTO user_accounts (id, email, password, profile_id, created_at, role)
        VALUES (
            v_user_id,
            format('usuario%1$s@example.com', i),
            hash_password,
            v_profile_id,
            created_time,
            'USER'
        ...
        INSERT INTO items (id, user_id, category_id, title, description, condition, estimated_value, is_available, is_service, location, created_at, updated_at)
        VALUES (
            v_item_id,
            v_profile_id,
            v_category,
            format('Artículo de prueba %s', i),
            format('Descripción generada automáticamente para el artículo %s.', i),
            'Bueno',
            round( (100 + random() * 900)::numeric, 2),
            TRUE,
            FALSE,
            format('Ubicación %s', i % 150),
            created_time,
            created_time
        );

        INSERT INTO item_images (item_id, image_url) VALUES
            (v_item_id, format('https://picsum.photos/id/%s/600/400', (i % 1000) + 1))
        ON CONFLICT DO NOTHING;

        INSERT INTO item_wishlist (item_id, wishlist_item) VALUES
            (v_item_id, format('Deseo extra #%s', (i % 5) + 1))
        ON CONFLICT DO NOTHING;
    END LOOP;
END
$$;
